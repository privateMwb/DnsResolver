...
static void two_hop_pointer_chain_resolves() {
    ...
    CHK(out.questions.size() == 3);
    for (const auto& question : out.questions) {
        CHK(question.qname.labels.size() == 2);
        CHK(question.qname.labels[0] == "example");
        CHK(question.qname.labels[1] == "com");
    }
}

// Executes all compression-pointer-chain test cases.
static void run_tests() {
    RUN(two_hop_pointer_chain_resolves);
}
