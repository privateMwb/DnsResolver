...
static void forward_pointer_rejected() {
...
static void valid_backward_pointer_accepted() {
    Vector<std::byte> base = buildBaseQuery();
    ...
    Status s = Parser::parse(buffer, out);
    CHK(s == Status::OK);
    CHK(out.questions.size() == 2);
    CHK(out.questions[1].qname.labels.size() == 2);
    CHK(out.questions[1].qname.labels[0] == "example");
    CHK(out.questions[1].qname.labels[1] == "com");
}

// Executes all compression pointer test cases.
static void run_tests() {
    RUN(forward_pointer_rejected);
    RUN(far_forward_pointer_rejected);
    RUN(valid_backward_pointer_accepted);
}
