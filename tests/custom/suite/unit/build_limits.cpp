...
static void oversized_label_rejected() {
...
static void oversized_section_rejected() {
    ...
    Vector<std::byte> bytes;
    Status s = Builder::build(message, bytes);
    CHK(s == Status::SECTION_TOO_LARGE);
}

// Verifies rdata too large for the 16-bit rdlength field is rejected.
static void oversized_rdata_rejected() {
    Packet::ResourceRecord record;
    record.name = makeName("example", "com");
    record.type = kType;
    record.rclass = kClass;
...
// Executes all build-limit test cases.
static void run_tests() {
    RUN(oversized_label_rejected);
    RUN(boundary_label_accepted);
    RUN(oversized_section_rejected);
    RUN(oversized_rdata_rejected);
}
