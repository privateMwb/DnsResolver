...
static void NameParse_Uncompressed(benchmark::State& state) {
    Vector<std::byte> bytes = buildUncompressedQuery();
...
BENCHMARK(NameParse_Uncompressed);

// Measures parse() on a name reached through a compression pointer.
static void NameParse_Compressed(benchmark::State& state) {
...
BENCHMARK(NameParse_Compressed);
