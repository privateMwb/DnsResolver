...
Status Parser::parse(std::span<const std::byte> buffer, Packet::Message& out) {
    ...
    if (Status s = parseHeader(buffer, offset, out.header); s != Status::OK)
        return s;
    ...
    for (std::uint16_t i = 0; i < out.header.qdcount; ++i) {
        Packet::Question q;
        if (Status s = parseQuestion(buffer, offset, q); s != Status::OK)
            return s;
    ...
    for (std::uint16_t i = 0; i < out.header.ancount; ++i) {
        ...
        if (Status s = parseResourceRecord(buffer, offset, rr); s != Status::OK)
            return s;
...
Status Parser::parseHeader(std::span<const std::byte> buffer, std::size_t& offset,
                           Packet::Header& out) {
    std::uint16_t id;
    if (Status s = readU16(buffer, offset, id); s != Status::OK)
        return s;
    ...
    if (Status s = readU16(buffer, offset, qdcount); s != Status::OK)
        return s;
    if (Status s = readU16(buffer, offset, ancount); s != Status::OK)
        return s;
    if (Status s = readU16(buffer, offset, nscount); s != Status::OK)
        return s;
    if (Status s = readU16(buffer, offset, arcount); s != Status::OK)
        return s;
...
// ============================================================
Status Parser::parseName(std::span<const std::byte> buffer, std::size_t& offset,
                         Packet::Name& out) {
    out.labels.clear();
    ...
    while (true) {
        if (stepsRemaining-- == 0)
            return Status::COMPRESSION_LOOP;
        ...
        if ((lengthByte & 0xC0) == 0xC0) {
            ...
            if (target >= cursor)
                return Status::COMPRESSION_FORWARD_POINTER;
        ...
        if ((lengthByte & 0xC0) != 0x00)
            return Status::MALFORMED_NAME;

        std::size_t labelStart = cursor + 1;
        std::size_t labelLen = lengthByte;
        if (labelStart + labelLen > buffer.size())
            return Status::BUFFER_TOO_SMALL;
...
Status Parser::parseQuestion(std::span<const std::byte> buffer, std::size_t& offset,
                             Packet::Question& out) {
    if (Status s = parseName(buffer, offset, out.qname); s != Status::OK)
        return s;
    if (Status s = readU16(buffer, offset, out.qtype); s != Status::OK)
        return s;
    if (Status s = readU16(buffer, offset, out.qclass); s != Status::OK)
        return s;
    return Status::OK;
}
...
Status Parser::parseResourceRecord(std::span<const std::byte> buffer, std::size_t& offset,
    ...
    if (Status s = parseName(buffer, offset, out.name); s != Status::OK)
        return s;
    if (Status s = readU16(buffer, offset, out.type); s != Status::OK)
        return s;
    if (Status s = readU16(buffer, offset, out.rclass); s != Status::OK)
        return s;
    if (Status s = readU32(buffer, offset, out.ttl); s != Status::OK)
        return s;

    std::uint16_t rdlength;
    if (Status s = readU16(buffer, offset, rdlength); s != Status::OK)
        return s;

    if (offset + rdlength > buffer.size())
        return Status::RDATA_LENGTH_MISMATCH;
